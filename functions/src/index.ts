...
//     if (manager.docs.length === 0) return response.status(201).send('Invalid Email Address');
//
...
export const getReservations = functions.https.onRequest((request, response) => {
    corsHandler(request, response, async () => {
        ...
        if (data.docs.length === 0) {
            return response.status(201).send('no docs')
        }
        ;
...
export const getOnlineOrder = functions.https.onRequest(async (request, response) => {
    corsHandler(request, response, async () => {
        ...
        const onlineOrder = await admin.firestore().collection('onlineOrders').where('branchID', '==', request.headers.branchid).get();
        if (onlineOrder.docs.length === 0) return response.status(201).send('No Online Orders');

        const list: any = [];
        onlineOrder.docs.map(el => list.push({id: el.id, data: el.data()}));
...
export const getItems  = functions.https.onRequest( async (request, response) => {
    corsHandler(request, response, async () => {
        ...
        const items = await admin.firestore().collection('Items').where('branchID', "==", request.headers.branchid).get();
        if (items.docs.length === 0) return response.status(201).send('No Items in database');
        const list: any = [];
